    ...
    <div className="p-2 md:p-0 animate-pulse">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          {/* Button skeleton - hidden on very small screens or smaller text */}
          <div className="h-10 w-28 bg-primary/20 rounded-xl" />
        </div>
      </div>

      {/* Template tabs skeleton */}
      <div className="flex gap-2 mb-4 overflow-hidden border-b border-border pb-0 md:pb-5">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="h-8 w-24 bg-slate-100 rounded-none border-b-2 border-transparent shrink-0" />
        ))}
      </div>
      ...
      <div>
        ...
        <div className="hidden md:block bg-white border border-gray-200 rounded-2xl overflow-hidden">
          <div className="border-b border-gray-100 px-5 py-4 flex gap-16">
            {["w-24", "w-32", "w-20", "w-16", "w-20"].map((w, i) => (
              <div key={i} className={`h-3 ${w} bg-slate-100 rounded`} />
            ))}
          </div>
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="border-b border-gray-50 px-5 py-4 flex gap-16">
