      ...
      {initialClients.length === 0 && !showForm ? (
        ...
        <div className="grid gap-4">
          {initialClients.map((c) => (
            <div
              key={c.id}
              className="bg-surface border border-border rounded-2xl p-5 hover:shadow-md hover:border-primary/20 transition-all group relative cursor-pointer"
            >
              ...
              <div className="flex flex-wrap gap-x-5 gap-y-1.5 text-[12px] text-muted">
                {c.contactPerson && <span className="font-medium text-foreground/80">{c.contactPerson}</span>}
                {c.phone && (
                  <span className="flex items-center gap-1">
                    <Phone size={11} className="text-primary/70" /> {c.phone}
                  </span>
                )}
                {c.email && (
                  <span className="flex items-center gap-1">
                    <Mail size={11} className="text-primary/70" /> {c.email}
                  </span>
                )}
                {c.address && (
                  <span className="flex items-center gap-1">
                    <MapPin size={11} className="text-primary/70" /> {c.address}
                  </span>
                )}
              </div>
            </div>
